...
//Inputs: setters and connections
//List<TaskInput> inputs =baseTask.getInputs();
//for(TaskInput input:inputs) {
//	if(input instanceof Setter ) {
//		Element setters = baseTaskElement.addElement("setters");
//	}
//	else if (input instanceof Connection) {
//		Element input_connectors = baseTaskElement.addElement("input_connectors");
//	}
//}

//List<AbstractTask> childrenTasks=baseTask.getChildren();
...
public class Utils {
	...
	public static void writeDocToXMLFile(Document document,String fileName) {
		...
		FileWriter out;
		try {
			out = new FileWriter("model//"+fileName);
			document.write(out);
			out.close();
		} catch (IOException e) {
			e.printStackTrace();
